...
def setup_database_registers_arb(station, test_exp, sweepers, time_independent = False):
    ...
    for sweeper in sweepers:
        ...
        if "res" in sweeper["name"]:
            meas_forward.register_parameter(channel.res, setpoints = (*independent_params,))
            meas_forward.register_parameter(channel.res2, setpoints = (*independent_params,))
